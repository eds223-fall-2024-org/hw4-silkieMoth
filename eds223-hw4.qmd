...
&emsp;&emsp;&emsp;a. Do extent, resolution, and CRS match between depth and temperature rasters?
```{r}
#| code-fold: true
...
&emsp;&emsp;&emsp;b. (Demonstration) When you attempt to stack two rasters with different resolutions, extents, and CRSs, you get an error.
```{r}
#| code-fold: true
...
```
&emsp;&emsp;&emsp;c. (Demonstration) Because the depth and temperature rasters are now matched in these respects, it is possible to stack them.
```{r}
#| code-fold: true
...
&emsp;&emsp;&emsp;b.  Compute area (km) by cell in habitat suitability raster.
```{r, eval=FALSE}
# get area per cell in suitability raster
suitability_km <- cellSize(suitability,
                           mask = TRUE,
...
&emsp;&emsp;&emsp;c.    Crossreference area per cell and and suitable EEZ zones to compute total suitable habitat area per zone.
```{r, eval=FALSE}
# get total suitable area by suitable eez zone
suitable_area <- zonal(suitability_km, 
                       vect(suitable_eez), 
...
| Data | Citation | Link |
|-------------------|----------------------------------|-------------------|
| OpenStreetMap from Geofabrik | Fellows I, Stotz utJlbJP (2023). _OpenStreetMap: Access to Open Street Map Raster Images_. R package version 0.4.0, https://CRAN.R-project.org/package=OpenStreetMap | [Link](https://www.openstreetmap.org) |
